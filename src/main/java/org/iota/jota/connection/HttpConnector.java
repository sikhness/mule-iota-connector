...
public class HttpConnector implements Connection {
	...
	/**
	 * Creates an HTTP connector using the provided url. This url must be complete
	 * including port and protocol <br/>
	 ...
	 * @param host     The host we use (Domain and optional subdomain)
	 * @param port     The port we use
	 * @param file     The file extension of the host (so "/node/" in
	 *                 iota.net/node/)
	 * @throws MalformedURLException if this is an invalid URL
	 */
	 ...
	 * @throws MalformedURLException if this is an invalid URL
	 */
	public HttpConnector(String protocol, String host, int port, String file, int timeout)
			throws MalformedURLException {
		this(new URL(protocol, host, port, file), timeout);
	}

	/**
	 * Mule IOTA Connector Modification: Implementing SSLSocketFactory in building
	 * connection to IOTA
	 */
	public HttpConnector(String protocol, String host, int port, String file, int timeout,
			SSLSocketFactory sslSocketFactory) throws MalformedURLException {
		this(new URL(protocol, host, port, file), timeout, sslSocketFactory);
	}
				...
				.addInterceptor(new Interceptor() {
					...
					public okhttp3.Response intercept(Chain chain) throws IOException {
						...
						newRequest = request.newBuilder()
								.addHeader(X_IOTA_API_VERSION_HEADER_NAME, X_IOTA_API_VERSION_HEADER_VALUE).build();
	 ...
	 * connection to IOTA
	 */
	public HttpConnector(URL url, int timeout, SSLSocketFactory sslSocketFactory) {
		Objects.requireNonNull(url, "URL cannot be null");
		client = new OkHttpClient.Builder().connectTimeout(timeout, TimeUnit.SECONDS)
				.writeTimeout(timeout, TimeUnit.SECONDS).readTimeout(timeout, TimeUnit.SECONDS)
				.addInterceptor(new Interceptor() {
	...
	/**
	 * Creates an HTTP connector using the provided HTTP client, with the default
	 * port
	 * 
	...
	protected static <T> Response<T> wrapCheckedException(final Call<T> call)
			throws ArgumentException, IllegalStateException, IllegalAccessError {
		try {
	...
	@Override
	public GetNodeAPIConfigurationResponse getNodeAPIConfiguration(IotaCommandRequest request)
			throws ArgumentException {
		final Call<GetNodeAPIConfigurationResponse> res = service
				.getNodeAPIConfiguration(IotaCommandRequest.createGetNodeAPIConfiguration());
		return wrapCheckedException(res).body();
	}
	...
	@Override
	public InterruptAttachingToTangleResponse interruptAttachingToTangle(IotaCommandRequest request)
			throws ArgumentException {
