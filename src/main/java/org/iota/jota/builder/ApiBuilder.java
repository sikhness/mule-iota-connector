...
public abstract class ApiBuilder<T extends ApiBuilder<T, E>, E extends IotaAPICore>
		extends AbstractBuilder<T, E, ApiConfig> implements ApiSettings {
	 ...
	 * Starts by checking a optionally set config using
	 * {@link #config(org.iota.jota.config.Config)} THen checks Environment, and in
	 * the end goes to {@link IotaDefaultConfig} for defaults
	 * 
	 * @return The builder
	...
	protected T generate() throws Exception {
		...
		if (hasLegacyOptions()) {
			...
			if (sslSocketFactory == null) {
				...
				nodes.add(new HttpConnector(protocol, host, port, path, timeout, sslSocketFactory));
			}
		}
	...
	/**
	 * Sets the legacy host field after checking if the address is valid (network
	 * check)
	 * 
	 * @param host The host to set
	 * @return The builder instance
